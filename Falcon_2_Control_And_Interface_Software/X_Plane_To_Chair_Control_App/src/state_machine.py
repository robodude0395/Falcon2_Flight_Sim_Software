...
class StateMachine:
    ...
    def execute(self, n_values, n_safe):
        if self.state:
            #Execute current state
            transition = self.state.execute(n_values, n_safe)
            ...
            #TAKE CARE OF LATER!!!
            #self.send_udp_data(UDP_IP, STATUS_UDP_PORT, self.GetCurrentState(), safe)
    ...
    while not keyboard.is_pressed("q"):
        ...
        if current_time >= target_time:
            ...
            if(temp_safe_state != None):
                safe_state = temp_safe_state[1]

            print(safe_state, " ", values)

            sm.execute(values, safe_state)
