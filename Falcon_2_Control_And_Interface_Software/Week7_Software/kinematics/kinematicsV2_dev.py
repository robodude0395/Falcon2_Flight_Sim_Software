...
class Kinematics(object):
    ...
    def point_at_distance(self, idx, d):
        ...
        p =  self.point_on_line(idx, d) 
        # print(p)        
        return x,y,0
    

    def point_on_line(self, idx, d):

        mn = self.slider_endpoints[idx][0]
        mx = self.slider_endpoints[idx][1]
        # print("idx", idx, mn,mx) 
        # Calculate the ratio of the distance between mn and mx to the distance between mn and the new point
        ratio = d / self.actuator_range
        # Calculate the coordinates of the new point
        x = (1 - ratio) * mn[0] + ratio * mx[0]
        y = (1 - ratio) * mn[1] + ratio * mx[1]
        return (x, y, 0)
...
if __name__ == "__main__":
    ...
    def plot_using_iter(pose):
        dists, carriage_points =  k.slider_pos_from_pose(pose)
        carriage_points = []
        for i in range(6):
             carriage_points.append( k.point_on_line(i, dists[i]))
        percents = k.percent_from_len(dists)    
        plot_config.plot3d_carriages(cfg, pose, carriage_points, k.slider_endpoints, percents)
